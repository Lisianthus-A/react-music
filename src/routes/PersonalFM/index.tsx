...
function PersonalFM() {
    ...
    useEffect(() => {
        // 隐藏外部音乐播放器
        // @ts-ignore
        const middleEl: HTMLDivElement = document.getElementsByClassName(layoutStyle.middle)[0];
        // @ts-ignore
        const bottomEl: HTMLDivElement = document.getElementsByClassName(layoutStyle.bottom)[0];
    ...
    return (
        <div className={style['personal-fm']}>
            ...
            <div className="music-box">
                ...
                <Link className="song" to={`/Song?id=${id}`} target="_blank" title={name}>
                    {name}
                </Link>
                <div className="progress-container" ref={progressContainerRef}>
                    <div className="progress" ref={progressRef} onClick={handleSetProgress} />
                </div>
                <div className="btn-container">
                    ...
                    <div className="icon" title='喜欢' onClick={handleLike}>
                        {like
                            ? <HeartFilled style={{ color: '#c62f2f' }} />
                            : <HeartOutlined />
                        }
                    </div>
