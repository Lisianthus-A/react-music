...
interface Props {
    ...
    onSetName: (...args: any[]) => void;
    onSetImg: (...args: any[]) => void;
}
...
function ModalView({ visible, onClose: handleClose, onSetName, onSetImg }: Props) {
    ...
    const onFinish = async (values: Record<string, any>) => {
        ...
        const { token } = result;
        //token 过期时间
        const maxAge = Number(result.cookie.match(/MUSIC_U=\w+;\s?Max-Age=(\d+)/)[1]);
